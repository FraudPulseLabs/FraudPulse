...
def load_data():
    ...
    y_train_full = pd.read_csv(DATA_DIR / "y_train.csv")["is_fraud"].astype(int)
...
def chronological_train_calibration_split(X, y, calibration_frac=0.2):
    """
    Keeps the split time-aware.

    The first 80% of X_train is used for model training.
    The last 20% of X_train is used for probability calibration.

    This avoids random leakage from future-like rows into earlier training rows.
    """
...
def find_best_fbeta_threshold(y_true, y_proba, beta=0.5):
    ...
    beta=0.5 means precision is weighted more than recall.
    This is useful when false positives are expensive.
    """
...
def find_ks_threshold(y_true, y_proba):
    ...
    Finds the score where separation between fraud and non-fraud is strongest.
    """
    scores = pd.DataFrame({
...
def generate_decision_thresholds(y_true, y_proba):
    ...
    The review threshold comes from KS.
    The decline threshold comes from F-beta.
    """
...
def save_feature_schema(X_train, metrics, thresholds):
    schema = {
        "model_name": "calibrated_lightgbm",
