...
*Version 1.2 (harmonized merge of Backlog v1.1 and User Stories & Acceptance Criteria v3)*

> **Merge & harmonization note:** This file merges two source documents:
> - `FraudPulse_Backlog_v1_1.docx` — the detailed, sprint-planning backlog (37 stories, FP-001…FP-037, across 14 epics). This is the primary content below.
> - `FraudPulse_User_Stories_Acceptance_v3.docx` — a concise, epic-level summary (US-1…US-6) originally written as a more human-readable companion piece.
>
> The concise US-1…US-6 stories are kept as a **quick-reference appendix** at the end of this document, cross-referenced to the epics/stories that implement them, since they describe the same system at a coarser grain and both add value for different audiences (see `CHANGELOG.md`).
>
> **Significant fixes applied during harmonization** (full detail in `CHANGELOG.md`):
> - Section 6 story-card IDs (FP-011 onward) had drifted out of sync with the Epic Overview, Sprint Allocation, Traceability Matrix, and Appendix A tables, including one duplicated ID (`FP-020` used for two unrelated stories). All story cards have been renumbered to match the canonical Appendix A / traceability numbering.
> - Epic **E9 — Transaction Lifecycle** (FP-025, FP-026) was referenced throughout the document's tables but had no story cards written. Both cards have been reconstructed and flagged inline for team review.
> - Story **FP-011 — Configure decision thresholds** and **FP-014 — Enable and disable fraud rules** were likewise referenced but missing; both reconstructed and flagged inline.
> - A subsection numbering gap (6.8 → 6.10, no 6.9) has been fixed.
...
MSc Software Engineering Capstone Project
...
|---|---|
| Version | 1.1 |
| Status | Revised — scope reduced for July delivery |
| Owner | Capstone team |
| Sprint cadence | 6 sprints x 2 weeks (3 months part-time) |
| Stories (v1.0) | 40 |
| Stories (v1.1) | 37 (6 epics removed, 14 new stories added from updated FRs) |

# 1. Introduction

FraudPulse is a simulated end-to-end fraud detection platform built as an MSc Software Engineering capstone. Version 1.1 of this backlog reflects a deliberate reduction in scope to ensure the project delivers a working, demonstrable system by July.

Six areas of complexity have been removed: dispute handling, chargeback decisioning, event queues, complex file-based reconciliation, model version management, and the advanced metrics dashboard. These are marked as deferred to v2 in section 3 and the traceability matrix. The remaining scope is designed to fit comfortably within the six-sprint plan at a part-time pace.

The functional requirements have also been updated (FR-1 through FR-19) and this backlog is fully aligned to the new numbering.

## 1.1 What changed from v1.0

- Removed: Disputes (E6), Chargebacks (E6), Back-office processing (E7), Reconciliation (E5), model version management (FP-037), advanced dashboard (FP-039, FP-040).

- Added from updated FRs: Authentication and RBAC (E1 — entirely new), expanded Rule-Based Detection (E5), expanded Watchlist Management (E6), simplified Transaction Lifecycle (E9), Reports (E11), Admin panel (E12), Testing and Simulation (E13), Audit and Logging (within E12).

- Scope change rationale: see section 3 (Removed Scope).

- Traceability matrix in section 8 maps every FR-1 to FR-19 requirement.
...
- **Must** — required for the July delivery. Removing it makes the system incoherent.
...
# 3. Removed scope (v1.0 → v1.1)

The following stories and epics were removed. Each will be reconsidered for a v2 delivery after the capstone submission.

| Epic / Area | Stories removed | Rationale |
|---|---|---|
| Disputes (E6 v1.0) | FP-023–FP-028 (6 stories, 14 pts) | Dispute handling requires additional lifecycle states, a separate UI panel, and eligibility-window logic that would occupy at least one full sprint. Deferred to v2. |
| Chargebacks (part of E6) | FP-027, FP-028 (5 pts) | Chargeback decisioning depends on disputes and reconciliation; removing the dependencies makes this impossible to implement correctly. Deferred to v2. |
| Back-office / Event queues (E7 v1.0) | FP-029–FP-031 (9 pts) | Event-driven incremental processing is the most infrastructure-heavy component. Without disputes and reconciliation it has no business value in this release. Deferred to v2. |
| Complex reconciliation (E5 v1.0) | FP-019–FP-022 (12 pts) | File-based settlement ingestion, batch comparison, and reconciliation issue routing form a standalone sub-system. Removing it eliminates the main driver of back-office complexity. Deferred to v2. |
| Model version management | FP-037 v1.0 (2 pts) | Version management adds deployment complexity with limited academic value. The model version string is still stored on each score record for traceability; switching versions requires a redeploy. |
| Advanced metrics dashboard | FP-039, FP-040 v1.0 (4 pts) | Replaced by FP-029 (metrics summary) which serves the same informational purpose with a much simpler implementation. |
...
## 6.1 Epic E1 — Authentication and User Management
...
## 6.2 Epic E2 — Transaction Ingestion
...
**2.** The API returns a JSON response containing the transaction ID, fraud score, and decision (ALLOW, REVIEW, or BLOCK) within 500 ms for the 95th percentile of test traffic.
...
**Primary persona:** Fraud Analyst / Admin  | Functional req:** FR-2.6

## 6.3 Epic E3 — Fraud Scoring
...
**5.** If scoring fails at runtime, the transaction is marked REVIEW, the error is logged, and a fallback alert is raised.
...
## 6.4 Epic E4 — Decision Engine
### FP-010 — ALLOW, REVIEW, and BLOCK classification
...
**1.** Default thresholds: ALLOW below 0.40, REVIEW from 0.40 to 0.79 inclusive, BLOCK at 0.80 or above.
...
**4.** REVIEW transactions are automatically added to the watchlist; BLOCK transactions raise an immediate alert.
...
> *Reconstructed story — this card was referenced in the Epic Overview, Sprint Allocation, Traceability Matrix, and Appendix A of the original v1.1 backlog, but no story card existed for it in Section 6. Written to match the surrounding cards' format and level of detail; team should review before sprint planning. See `CHANGELOG.md`.*

**Story:** As an administrator, I want to configure the ALLOW/REVIEW/BLOCK score thresholds, so that the sensitivity of the decision engine can be tuned without a code change.

**Acceptance criteria:**

**1.** An admin-only endpoint and UI form accept new threshold values for the REVIEW and BLOCK boundaries.

**2.** The system rejects invalid threshold configurations (e.g., REVIEW boundary ≥ BLOCK boundary).
...
## 6.5 Epic E5 — Rule-Based Fraud Detection
...
**2.** A triggered rule raises the effective decision to at least REVIEW regardless of the model score.
...
**Story:** As FraudPulse, I want to flag transactions from risky merchants, flagged users, or unusual locations, so that contextual risk signals supplement the model score.
...
**4.** Contextual flags follow the same escalation logic as other rules (at least REVIEW).
...
> *Reconstructed story — see note on FP-011 above; this card was likewise referenced in overview/traceability tables but missing from Section 6.*
...
## 6.6 Epic E6 — Watchlist Management
...
**1.** BLOCK decisions add the transaction to the blacklist; REVIEW decisions add it to the watchlist with the triggering reason.
...
**Primary persona:** System / Fraud Analyst  | Functional req:** FR-6.1, FR-6.2, FR-6.5, FR-6.6
...
**Primary persona:** Fraud Analyst  | Functional req:** FR-6.3, FR-6.4, FR-6.7
...
**Primary persona:** System  | Functional req:** FR-6.8

## 6.7 Epic E7 — Alert Generation
...
**1.** An alert is created for every REVIEW and BLOCK decision and for every rule trigger on an ALLOW transaction that came close to the threshold.
...
### FP-019 — Alert queue management

**Priority:** Must  |  **2 pts | S5**

**Story:** As a fraud analyst, I want a queue of open alerts ordered by severity and age, so that I can work through them in priority order.

**Acceptance criteria:**

**1.** The queue defaults to status NEW, ordered by severity (HIGH first) then by age.

**2.** Acknowledging an alert changes its status to ACKNOWLEDGED and records the analyst and timestamp.

**3.** Resolving an alert requires a short resolution note; status moves to RESOLVED.

**4.** Filters by severity, status, and date range are available.

**Primary persona:** Fraud Analyst  | Functional req:** FR-7.5, FR-7.6

### FP-020 — Escalate alerts to cases

**Priority:** Must  |  **2 pts | S4**

**Story:** As a fraud analyst, I want to escalate one or more related alerts into a case, so that connected suspicious activity is investigated together.

**Acceptance criteria:**

**1.** An analyst can select one or more alerts and create a case with a title and optional description.

**2.** All selected alerts are linked to the new case and their status is updated to ACKNOWLEDGED.

**3.** Creating a case from an alert is also available from the alert detail view.

**4.** A single alert cannot belong to more than one case; attempting to add a linked alert to a second case returns a clear error.

**Primary persona:** Fraud Analyst  | Functional req:** FR-7.7, FR-8.1, FR-8.2

## 6.8 Epic E8 — Case Management
### FP-021 — Create and view cases
...
### FP-023 — Case status workflow and assignment
...
## 6.9 Epic E9 — Transaction Lifecycle

> *Reconstructed section — Epic E9 was listed in the Epic Overview, Sprint Allocation, and Traceability Matrix of the original v1.1 backlog (mapped to FR-9 and stories FP-025/FP-026), but Section 6 jumped directly from Epic E8 to a section mislabeled "6.10 Epic E10" with no Epic E9 content at all. It also reused the IDs FP-025/FP-026 for two unrelated Dashboard stories elsewhere in the document. Both story cards below are reconstructed from the Appendix A titles/estimates and the FR-9 requirement text; the team should review and refine before sprint planning. See `CHANGELOG.md` for the full account of this fix.*

### FP-025 — Track transaction lifecycle

**Priority:** Must  |  **2 pts | S2**

**Story:** As FraudPulse, I want to track a transaction's lifecycle state beyond the initial decision, so that downstream settlement status is visible alongside the fraud decision.

**Acceptance criteria:**

**1.** Each transaction record has a lifecycle state field with at minimum the values `AUTHORIZED` and `SETTLED`.

**2.** A transaction is created in the `AUTHORIZED` state when the decision engine returns ALLOW or REVIEW.

**3.** An API endpoint or scheduled job can transition a transaction to `SETTLED`.

**4.** Lifecycle transitions are timestamped and stored as part of the transaction history.

**Primary persona:** System | Functional req:** FR-9.1

### FP-026 — View transaction lifecycle history

**Priority:** Should  |  **1 pt | S5**

**Story:** As a fraud analyst, I want to see a transaction's lifecycle timeline in its detail view, so that I can understand its full history alongside the fraud decision.

**Acceptance criteria:**

**1.** The transaction detail view (FP-022) shows a chronological timeline of lifecycle state changes with timestamps.

**2.** The timeline is included in the drill-down from the Transaction Monitor (FP-027).

**3.** If no state changes exist beyond creation, the timeline shows the single `AUTHORIZED` entry.

**Primary persona:** Fraud Analyst | Functional req:** FR-9.2, FR-15.7

## 6.10 Epic E10 — Dashboard and UI
### FP-027 — Transaction monitor
...
**2.** Filters by decision (ALLOW/REVIEW/BLOCK), user ID, score range, and date range are available and combinable.
...
### FP-028 — Alert and case dashboard
...
**1.** The summary shows: transactions processed in the last hour and 24 hours, decision breakdown (ALLOW/REVIEW/BLOCK count and percentage), active cases, and score mean and median.
...
## 6.11 Epic E11 — Reports and Analytics
### FP-030 — Fraud summary report
...
**1.** The report includes: total transactions, counts and percentages for ALLOW/REVIEW/BLOCK decisions, alerts generated, cases opened and closed, and model performance metrics (precision and recall from offline evaluation).
...
**Primary persona:** Fraud Analyst / Admin  | Functional req:** FR-16.1 to FR-16.7, FR-16.10

## 6.12 Epic E12 — Administration
### FP-031 — Admin configuration panel
...
**1.** The panel allows editing: ALLOW/REVIEW/BLOCK score thresholds, per-rule enable/disable toggles, rule-specific parameters (e.g., velocity window in minutes, high-amount limit), and case severity level definitions.
...
**4.** A "Preview" mode allows an admin to see how existing transactions would be reclassified under proposed thresholds before applying.

**Primary persona:** System Administrator  | Functional req:** FR-17.1, FR-17.2, FR-17.5, FR-17.7

### FP-032 — System logs and audit trail
...
## 6.13 Epic E13 — Testing and Simulation
### FP-033 — Generate demo transactions
...
## 6.14 Epic E14 — Platform and DevOps
### FP-035 — CI/CD pipeline
...
### FP-036 — Containerised deployment
...
### FP-037 — Auto-generated API documentation
...
- All acceptance criteria are demonstrated in the sprint review or covered by automated tests.
...
| FP-010 | ALLOW, REVIEW, and BLOCK classification | Must | 3 | 3 |
...
*A concise, epic-level view of the same system, originally maintained as a standalone companion document. Each story below maps to one or more epics/stories in the detailed backlog above.*

## US-1 Transaction Ingestion — *maps to Epic E2 (FP-004–FP-006)*
...
## US-2 Fraud Scoring — *maps to Epic E3 (FP-007–FP-009)*
...
- **AC-3:** Given scoring fails, when fallback rules apply, then the transaction is routed to REVIEW and the failure is logged.

## US-3 Decision Engine — *maps to Epic E4 (FP-010, FP-011)*

**User Story:** As a System, I want to classify transactions as ALLOW, REVIEW, or BLOCK so response actions are timely and policy-aligned.

**Acceptance Criteria:**
- **AC-1:** Given a score below the allow threshold, when decisioning runs, then the transaction outcome is ALLOW.
- **AC-2:** Given a score between configured thresholds, when decisioning runs, then the outcome is REVIEW.
- **AC-3:** Given a score above block threshold, when decisioning runs, then the outcome is BLOCK and an alert is raised.

## US-4 Case Management — *maps to Epic E8 (FP-021–FP-024)*
...
## US-5 Dashboard and Monitoring — *maps to Epic E10 (FP-027–FP-029)*
...
## US-6 Access and Audit — *maps to Epics E1 and E12 (FP-001–FP-003, FP-031, FP-032)*
...
- **AC-3:** Given an audit review is requested, when logs are queried, then complete action history is available.
