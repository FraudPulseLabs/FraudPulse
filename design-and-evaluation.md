...
![FraudPulse Architecture](.github/images/architecture-diagram.png)

_Figure 1. High-level system architecture._
...
![FraudPulse Data Flow](.github/images/data-flow-diagram.png)

_Figure 2. Data flow for transaction processing and investigation._

### Transaction Processing (Processes 1–3)

The issuing bank submits a transaction to **Process 1 (Process Transaction)**, where the request is validated. The system checks the merchant against the watchlist and, if the merchant is blacklisted, immediately returns a **Decline** decision.

Otherwise, **Process 2 (Fraud Scoring)** retrieves the customer's historical transaction data, builds the feature vector, and evaluates the transaction using the LightGBM fraud detection model to generate a fraud score. The score is compared with the configured decision thresholds, after which the transaction and fraud score are stored in the database and a decision (**Approve**, **Review**, or **Decline**) is returned to the issuing bank.

### Alert Handling and Investigation (Processes 3–4)

Transactions requiring further attention trigger **Process 3 (Alert & Case Management)**. Depending on the decision, the system creates an alert, a case, or both, and stores the associated records.

A fraud analyst then performs **Process 4 (Investigation)** by reviewing the transaction details, fraud score, and model explanations (SHAP values), assigning the case if necessary, adding investigation notes, and recording the final resolution.

Any updates made during the investigation are written back to the **Cases & Alerts** data store, providing a complete audit trail of the investigation lifecycle.
...
The fraud detection model is trained on a synthetic transaction dataset generated by `backend/ml/dataset_generator.py`.

A synthetic dataset was chosen because publicly available fraud datasets are typically anonymised and PCA-transformed, making them unsuitable for explainable fraud detection. This approach provides:

- Interpretable features for SHAP explanations.
...
Detailed descriptions of the datasets used in this project are available in the following documentation:

#### Raw Data
Descriptions of the original transaction data generated by `backend/ml/dataset_generator.py` can be found in:

- `backend/ml/data/raw/transactions_data_dictionary.md`

#### Processed Data
Descriptions of the features generated after preprocessing and feature engineering, which are used as input to the model, can be found in:

- `backend/ml/data/processed/model_features_data_dictionary.md`
...
- High-cardinality identifiers (e.g., merchant and card IDs) are target encoded using fraud rates, with global averages used for unseen values.
- Operational categorical fields (card type, channel, authentication method, transaction type) are ordinal encoded using precomputed mappings.
- Time features are converted into cyclical sine/cosine representations to preserve temporal patterns.
- Redundant features (such as `is_weekend`) are removed to reduce collinearity.
- Training and test data are split chronologically to prevent data leakage.

---

### 3.3 Class Imbalance

Fraud represents a small proportion of transactions, so the model applies:

- LightGBM class weighting to penalise fraud misclassification.
- Tuned decision thresholds for **Approve**, **Review**, and **Decline** outcomes.
- Platt scaling to calibrate predicted fraud probabilities.

---

### 3.4 Feature Engineering

Features are generated by the `FeatureBuilder` component across six categories.

| Category          | Example Features                                                                                                                       | Purpose                                                       |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| Temporal          | `hour_sin`, `hour_cos`, `dow_sin`, `dow_cos`, `is_night`                                                                               | Capture time-based fraud patterns using cyclical encoding.    |
| Geolocation       | `cross_border`                                                                                                                         | Identify higher-risk international transactions.              |
| Spend History     | `card_txn_count_prior`, `card_avg_amount_usd_prior`, `amount_vs_card_avg`, `seconds_since_last_txn`                                    | Detect deviations from normal customer behaviour.             |
| Velocity          | `txn_count_1h`, `txn_count_24h`                                                                                                        | Detect rapid transaction bursts associated with fraud.        |
| Categorical       | `cvv2_result_enc`, `avs_result_enc`, `authentication_enc`, `card_type_*`, `channel_*`                                                  | Capture transaction verification and payment characteristics. |
| Composite Signals | `is_amount_spike_vs_card_history`, `is_cross_border_amount_spike`, `is_velocity_burst_last_1h`, `is_weak_auth_on_above_average_amount` | Combine multiple indicators into stronger fraud signals.      |
...
| Feature                  | Reason                                                                             |
| ------------------------ | ---------------------------------------------------------------------------------- |
| `merchant_id`, `card_id` | Used only for history lookup and target encoding, not as model inputs.             |
| `is_weekend`             | Redundant after cyclical day-of-week encoding.                                     |
| `user_ip`, `device_info` | Excluded due to privacy concerns and inconsistent availability.                    |
| `is_fraud`               | Ground-truth label available only after investigation and never during prediction. |
...
User authentication is handled entirely by **Supabase Auth (GoTrue)**. The backend does not issue, store, or manage passwords or access tokens—it only verifies JWTs.

#### 4.1.1 Login Flow

1. The user signs in through the Angular frontend.
2. Supabase returns a signed JWT access token.
3. The Angular `authInterceptor` attaches the token to API requests as an `Authorization: Bearer <token>` header.
4. FastAPI validates the token using the `get_current_user` dependency before allowing access to protected endpoints.
...
- **ES256/RS256 (default):** Verifies tokens using Supabase's public JWKS endpoint, with keys cached to reduce network requests.
- **HS256 (legacy):** Uses the shared secret when `SUPABASE_JWT_SECRET` is configured.

For both methods, the JWT signature, audience, and expiry are validated. Invalid or expired tokens result in a **401 Unauthorized** response.

---

### 4.3 API Security

#### 4.3.1 Protected Endpoints

All secured endpoints depend on `get_current_user`, which:
...
- Provides authenticated user information (`id` and `email`) to downstream handlers.

#### 4.3.2 Request Validation

Input validation is performed using **Pydantic** schemas, ensuring requests conform to expected types before reaching business logic. JWT claims are also validated to prevent malformed or forged tokens.

#### 4.3.3 CORS

Cross-Origin Resource Sharing (CORS) is configured at the FastAPI application level and should allow requests only from trusted frontend origins.
...
| Variable              | Purpose                                                |
| --------------------- | ------------------------------------------------------ |
| `SUPABASE_URL`        | Used to derive the JWKS endpoint for JWT verification. |
| `SUPABASE_JWT_SECRET` | Legacy HS256 signing secret (optional).                |
| `SUPABASE_JWT_AUD`    | Expected JWT audience claim.                           |
| `DATABASE_URL`        | Database connection string.                            |

For automated testing, `DATABASE_URL` points to an in-memory SQLite database. In production, secrets should be managed through a secure secrets store and never committed to source control.
...
### 5.2 Backend — Integration Tests (pytest)
...
### 5.4 RAG — Evaluation & Smoke Tests
...
### 5.6 Continuous Integration Summary
